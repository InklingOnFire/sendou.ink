...
const TranslatePage: React.FC<RouteComponentProps> = () => {
  ...
  const getPercentage = (keyToCheck: any) => {
    ...
    for (const keyValue of Object.entries(english[keyToCheck] ?? {})) {
      if (keyValue[1]) englishCount++
    }

    for (const keyValue of Object.entries(toTranslate[keyToCheck] ?? {})) {
      if (keyValue[1]) translatedCount++
    }
